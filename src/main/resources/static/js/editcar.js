...
let idCar = Number.parseInt(idStr, 10)
...
// Thêm các option vào biến strStatus
    if (currentStatus !== "AVAILABLE") {
        strStatus += `<option value="0" class="text-success">AVAILABLE</option>`;
    } else {
        strStatus = `<option value="0" selected class="text-success">AVAILABLE</option>` + strStatus;
    }

    if (currentStatus !== "BOOKED") {
        strStatus += `<option value="1" class="text-warning">BOOKED</option>`;
    } else {
        strStatus = `<option value="1" selected class="text-warning">BOOKED</option>` + strStatus;
    }

    if (currentStatus !== "STOPPED") {
        strStatus += `<option value="2" class="text-danger">STOPPED</option>`;
    } else {
        strStatus = `<option value="2" selected class="text-danger">STOPPED</option>` + strStatus;
    }


    statusCar.innerHTML = strStatus
//basic
...
function displayDetail(data) {
    console.log(data)
    mileageCar.value = data.mileage
    consumption.value = data.consumption
    ...
    street.value = data.street.street
...
detailCar.addEventListener("click", async function () {
    ...
    await fetch(`http://localhost:8080/api/v1/car/${idCar}/cardetail`)
        .then(response => response.json())
